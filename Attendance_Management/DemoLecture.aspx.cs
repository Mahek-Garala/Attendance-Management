...
{
    public partial class DemoLecture : System.Web.UI.Page
    {
                ...
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["attendanceDatabase"].ConnectionString))
                {
                    try
                    {
                        ...
                        if (sub_id > 0)
                        {
                            ...
                            {
                                case "1":
                                    updatedquery = "SELECT phase1_lec_total FROM Attend WHERE subject_ID = @sub_id";
                                    break;
                                case "2":
                                    updatedquery = "SELECT phase2_lec_total FROM Attend WHERE subject_ID = @sub_id";
                                    break;
                                case "3":
                                    updatedquery = "SELECT phase3_lec_total FROM Attend WHERE subject_ID = @sub_id";
                                    break;
                            }
                            ...
                            if (!string.IsNullOrEmpty(updatedquery))
                            {
                                using (SqlCommand cmd = new SqlCommand(updatedquery, con))
                                {
                                    ...
                                    using (SqlDataReader reader = cmd.ExecuteReader())
                                    {
                                        if (reader.Read())
                                        {
                                            txtTotalLecture.Text = reader[0].ToString();
                                        }
                                    }
                                }
                            }
        ...
        private void UpdateLectureCount(int increment)
        {
            string subjectName = ddlSubject.SelectedValue;
            ...
            if (!string.IsNullOrEmpty(subjectName) && !string.IsNullOrEmpty(selectedPhase))
            {
                ...
                {
                    case "1":
                        columnName = "phase1_lec_total";
                        break;
                    case "2":
                        columnName = "phase2_lec_total";
                        break;
                    case "3":
                        columnName = "phase3_lec_total";
                        break;
                }
        ...
        private void UpdateAttendanceInDatabase(string studentID, string subjectName, string selectedPhase)
        {
            ...
            using (SqlConnection con = new SqlConnection(connString))
            {
                ...
                if (selectedPhase == "1")
                {
                    attendanceField = "phase1_lec_present";
                }
                else if (selectedPhase == "2")
                {
                    attendanceField = "phase2_lec_present";
                }
                else if (selectedPhase == "3")
                {
                    attendanceField = "phase3_lec_present";
                }

                // Update attendance for the student and subject
                string updateQuery = $"UPDATE Attend SET {attendanceField} = {attendanceField} + 1 WHERE student_ID = @studentID AND subject_ID = @subjectID";
